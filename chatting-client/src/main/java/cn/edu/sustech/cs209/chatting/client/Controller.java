...
public class Controller implements Initializable {
  ...
  public static final ConcurrentHashMap<String, List<String>> FileMap = new ConcurrentHashMap<>();
  ...
  public void Upload() {
    ...
    if (selectedFile != null) {
      ...
      SerFile serFile = new SerFile(fileName, groupName, fileBytes);
      try {
        sendMessageQueue.put(serFile);
      } catch (InterruptedException e) {
        e.printStackTrace();
      }
    }
  ...
  public void Download() {
    ...
    String groupName = chatList.getSelectionModel().getSelectedItem().getGroupName();
    if (!FileMap.containsKey(groupName)) {
      FileMap.put(groupName, new ArrayList<String>());
    }
    List<String> files = FileMap.get(groupName);
    ...
    String fileName = user.get();
    Message message = new Message(username, "server", groupName, "FILE " + fileName);
    System.out.println(message.getSentBy() + " " + message.getSendTo() + " " + message.getGroupName() + " " + message.getData());
    try {
      sendMessageQueue.put(message);
    } catch (InterruptedException e) {
      e.printStackTrace();
